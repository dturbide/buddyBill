-- Script pour corriger les problèmes avec les tables de groupes en utilisant un UUID fixe

-- 0. Définir un UUID de test pour simuler un utilisateur
-- Remplacez ceci par votre propre UUID d'utilisateur si vous le connaissez
DO $$
BEGIN
    ...
    -- Créer une fonction temporaire pour simuler auth.uid()
    CREATE OR REPLACE FUNCTION public.get_test_user_id()
    RETURNS UUID AS $$
    BEGIN
        -- Vous pouvez remplacer cet UUID par celui de votre utilisateur réel
...
-- 9. Vérifier si nous avons des utilisateurs dans la table auth.users
SELECT 'Utilisateurs disponibles:' as info;
SELECT id, email FROM auth.users LIMIT 10;

-- 10. Tester l'insertion d'un groupe avec un UUID d'utilisateur existant
-- Remplacez l'UUID ci-dessous par celui d'un utilisateur réel de votre base de données
...
BEGIN
    ...
    IF user_id IS NULL THEN
        RAISE EXCEPTION 'Aucun utilisateur trouvé dans auth.users. Impossible de créer un groupe.';
    END IF;
    
    -- Insérer un groupe avec cet utilisateur
    ...
    -- Insérer l'utilisateur comme membre admin du groupe
    ...
    RAISE NOTICE 'Groupe créé avec succès! ID: %', group_id;
END $$;

-- 11. Vérifier que le groupe a été créé
SELECT 'Groupes après insertion:' as info;
SELECT * FROM public.groups WHERE name = 'Groupe Test Script';

-- 12. Vérifier les membres du groupe
SELECT 'Membres du groupe:' as info;
SELECT gm.*, g.name as group_name
FROM public.group_members gm
JOIN public.groups g ON g.id = gm.group_id
WHERE g.name = 'Groupe Test Script';
